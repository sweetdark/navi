...
REALTYPE RTreeRectSphericalVolume( RTREEMBR *rc )
{
	...
	//radius = UeBase::CMathBasic::fastDoubleSqrt(sum_of_squares);
...
//    return 1;
//}
void RTreeSerial(RTREENODE *node, FILE *file, int *cursor)
{
  fwrite(&(node->count), sizeof(node->count), 1, file);
  fwrite(&(node->level), sizeof(node->level), 1, file);
  (*cursor) += sizeof(node->count) + sizeof(node->level);

  for(int i = 0; i < node->count; i++)
  {
    fwrite(&(node->branch[i].mbr.bound[0]), RealTypeSize, 2*DIMS_NUMB, file);
    (*cursor) += 2 * DIMS_NUMB * RealTypeSize;

    if(node->level == 0)
    {
      fwrite(&(node->branch[i].child), sizeof(node->branch[i].child), 1, file);
      (*cursor) += sizeof(node->branch[i].child);
    }
    else
    {
      RTreeSerial(node->branch[i].child, file, cursor);
    }
  }
}

void RTreeUnSerial(RTREENODE *node, FILE *file)
{
  ...
  for(int i = 0; i < node->count; i++)
  {
    fread(&(node->branch[i].mbr.bound[0]), RealTypeSize, 2*DIMS_NUMB, file);

    if(node->level == 0)
    {
      fread(&(node->branch[i].child), sizeof(node->branch[i].child), 1, file);
    }
    else
    {
      node->branch[i].child = RTreeNewNode();
      RTreeUnSerial(node->branch[i].child, file);
    }
  }
}

void RTreeUnSerialEx(RTREENODE *node, unsigned char *data)
{
  int cursor = 0;
  memcpy(&(node->count), (void *)(data + cursor), sizeof(node->count));
  cursor += sizeof(node->count);
  memcpy(&(node->level), (void *)(data + cursor), sizeof(node->level));
  cursor += sizeof(node->level);

  for(int i = 0; i < node->count; i++)
  {
    memcpy(&(node->branch[i].mbr.bound[0]), (void*)(data + cursor), RealTypeSize*2*DIMS_NUMB);
    cursor += RealTypeSize*2*DIMS_NUMB;

    if(node->level == 0)
    {
      memcpy(&(node->branch[i].child), (void*)(data + cursor), sizeof(node->branch[i].child));
    }
    else
    {
      node->branch[i].child = RTreeNewNode();
      RTreeUnSerialEx(node->branch[i].child, data);
    }
  }
}

int RTreeSearchCallback(int id, void* pfnParam)
{
  CMemVector *ids = (CMemVector *)pfnParam;
  ids->Add(&id);
