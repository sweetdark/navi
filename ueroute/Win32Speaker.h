...
#ifndef __IFLY_IS4_TEAM__2005_09_11__WIN32_SPEAKER__H__
#define __IFLY_IS4_TEAM__2005_09_11__WIN32_SPEAKER__H__
...
#define PCM_BLOCK_ALIGN(wBitsPerSample)	(((wBitsPerSample)+7)>>3)

#define PCM_BYTES_PER_SEC(nChannels,nSamplesPerSec,nBlockAlign)	\
					((nChannels)*(nSamplesPerSec)*(nBlockAlign))
...
//#define WAVE_FORMAT_DIVX_AUDIO160			0x0160 // DivX Audio
//#define WAVE_FORMAT_DIVX_AUDIO161			0x0161 // DivX Audio
...
private:
	CRITICAL_SECTION	m_tWork;
	HANDLE				m_hFull, m_hEmpty, m_hCacheFull, m_hCacheEmpty;
	LPWAVEFORMATEX		m_pWaveFormat;
	HWAVEOUT			m_hWaveOut;
	WAVEHDR				m_tWaveHeader[2];
	DWORD				m_nMaxSize, m_nSize[WAVE_COUNT + WAVE_EXT];
	LPSTR				m_pBuffer[WAVE_COUNT + WAVE_EXT + 1];
	DWORD				m_iRead, m_iWrite, m_nWrite;
	BOOL				m_bPlaying, m_bCaching, m_bPausing;
	short m_formatSize;
	short m_dataSize;
...
private:
	static void CALLBACK waveOutProc( HWAVEOUT, UINT, CWin32Speaker*, LPWAVEHDR, DWORD );

	void __Start();
	void __Stop();

	void __Open( LPWAVEFORMATEX );
	void __Close(bool isRelease = false);
...
public:
	...
	// 释放
	virtual ~CWin32Speaker();

	// 设置格式
	void SetWaveFormat( LPWAVEFORMATEX wfx );

	// 追加数据
	void Append( LPCVOID lpData, DWORD dwSize );
	...
	// 等待
	BOOL Wait( DWORD dwMsTimeout = INFINITE );
...
#endif /* !__IFLY_IS4_TEAM__2005_09_11__WIN32_SPEAKER__H__ */
