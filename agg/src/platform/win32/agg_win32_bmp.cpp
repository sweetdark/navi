...
namespace agg
{
	...
	void pixel_map::copy_bmp(int8u *dest, unsigned width, unsigned height, const RECT *device_rect, const RECT *bmp_rect) const
    {
		unsigned i, j;
		LONG addr, addr2;
		...
		else
		{
			...
			{
//#if _IS_PC
				if((i+device_rect->top < 0) || (height <= i+device_rect->top)) continue;
//#else
//				if((device_rect->bottom-i) >= height ) continue;
//				if((device_rect->bottom-i-1) <= 0) continue;
//#endif

				for(j=0; j<bmp_width; j++)
				{
					if((device_rect->left+j < 0) || (width <= device_rect->left+j))
					{
						continue;
					}
//#if _IS_PC
					addr = ((j+device_rect->left)*2) + ((i+device_rect->top)*width*2);
//#else
//					addr = ((j+device_rect->left)*2) + ((device_rect->bottom-i-1)*width*2);
//#endif
					addr2 = ((bmp_rect->left+j)*2)  + ((i*m_bmp->bmiHeader.biWidth)*2);

					
					unsigned rgb = *(int16u*)(m_buf+addr2);
					if(rgb == 0x3e0)	continue;		// 0x3e0 is green
					//if(rgb == 0x1ce7)	continue;		// is gray..:)
					
//#if !_IS_PC	// 555 to 565
//	                *(int16u*)(dest+addr) = (int16u)(((rgb << 1) & 0xFFC0) | (rgb & 0x1F));
//#else
					*(int16u*)(dest+addr) = (int16u)rgb;
//#endif

					//memcpy(dest + addr, m_buf + addr2, 2);
					//*(dest+addr) = 0x00;
					//*(dest+addr+1) = 0x00;
				}
			}
...
// 			for(i=0; i<240; i++)
// 			{
// 				*(dest + yPitch*2*j + i*2) = 0x00;
// 				*(dest + yPitch*2*j + i*2+1) = 0x00;
// 			}
...
#error "No appropriate constant for alpha format. Check version of wingdi.h, There must be AC_SRC_ALPHA or AC_SRC_NO_PREMULT_ALPHA"
	...
	**/
    ...
    bool pixel_map::load_from_memory_no_BITMAPFILEHEADER(char* _io_buffer, int _io_max, NavPictureBasic* _info)
    {
        //BITMAPFILEHEADER  bmf;
        BITMAPINFO       *bmi = 0;
        //unsigned          bmp_size;

		int				off = 0, yy, stride, col_size=0;

		unsigned char* _buf;

//		if ((off+sizeof(bmf)) > _io_max)	return false;

//		memcpy(&bmf, _io_buffer+off, sizeof(bmf));
//		off += sizeof(bmf);
        //fread(&bmf, sizeof(bmf), 1, fd);
//        if(bmf.bfType != 0x4D42) goto bmperr1;

//        bmp_size = bmf.bfSize - sizeof(BITMAPFILEHEADER);

        //bmi = (BITMAPINFO*) new unsigned char [bmp_size];
		bmi = (BITMAPINFO*)(_io_buffer + off);
		...
		col_size = calc_palette_size(bmi);
		if (col_size == 0)
		{
			if (m_bpp == 16)
				_info->m_clrType = PCS_RGB555;
			else
			if (m_bpp == 24)
				_info->m_clrType = PCS_BGR;
			else
			if (m_bpp == 32)
				_info->m_clrType = PCS_BGR;
		}
		...
		{
			_info->m_clrType = PCS_RGBPAL;
		}

        //create_from_bmp(bmi, _info);
...
//                      bmp->bmiHeader.biHeight;
        ...
        //m_bmp       = bmp;
        _buf       = calc_img_ptr(bmi);

		_info->m_bmpInfo = bmi;
		stride = GetStride(_info->m_width, _info->m_bpp);
		_info->m_rows = reinterpret_cast<unsigned char **>(::malloc(sizeof(unsigned char *) * _info->m_height));
		for (yy=0; yy<_info->m_height; yy++)
		{
   ...
   // bmperr1:
    ...
    bool pixel_map::load_file(LPCTSTR filename, NavPictureBasic* _info)
    {
        BITMAPFILEHEADER  bmf;
        //unsigned          bmp_size;
			...
			if (fread(&bmf, 1, sizeof(bmf), fd) == sizeof(bmf))
			{
		        if(bmf.bfType == 0x4D42)
				{
					...
					if (io_buff)
					{
						...
						{
							ret = load_from_memory_no_BITMAPFILEHEADER(io_buff, len, _info);
						}
					}
					if (!ret && io_buff)
						free(io_buff);
    ...
    //void pixel_map::create_from_bmp(BITMAPINFO *bmp, XBMP_INFO* _info)
    //{
    //    if(bmp)
    //    {
    //        m_img_size  = calc_row_len(bmp->bmiHeader.biWidth, 
