...
namespace agg
{
    ...
    { 
		// Herman:
		// Fix memory leak issue. However, below simple solution should affect performance little
		// ...
		//if(m_num_blocks)
		//{
		//	cell_aa** ptr = m_cells + m_num_blocks - 1;
		...
		//m_sorted_cells.remove_all();
		//m_sorted_y.remove_all();
    ...
    void outline_aa::sort_cells()
    {
        ...
        while(nb--)
        {
            ...
            while(i--) 
            {
				...
				// Herman
				// ...
				if(cur_y.start + cur_y.num > m_num_cells)
				{
					continue;
				}
