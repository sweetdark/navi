...
// Directly keep consistence with NavView model. However, if this class is refrenced by outside, this head file
// should be the one locating in "..\header\navview\viewdef.h"
...
namespace agg
{
    ...
    public:
	    ...
	    bool		load_from_memory_no_BITMAPFILEHEADER(char* _io_buffer, int _io_max, NavPictureBasic* _info);
	    bool		load_file(LPCTSTR filename, NavPictureBasic* _info);
		...
		void		copy_bmp(unsigned char *dest, unsigned width, unsigned height,
					const RECT *device_rect, const RECT *bmp_rect) const;
    ...
    private:
	    ...
	    //void create_from_bmp(BITMAPINFO *bmp, XBMP_INFO* _info);
